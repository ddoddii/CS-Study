...
내부 인터럽트에는 예외와 시스템콜이 있다. 외부 인터럽트에는 타이머 인터럽트, I/O 인터럽ㅌ, 전원 이상 인터럽트가 있다.
...
2. 유저모드에서 커널 모드로 변경
3. 기존 프로세스의 현재 컨텍스트를 PCB에 저장
4. 다음에 실행할 프로세스의 상태정보를 PCB에서 복구한 후 다음 프로세스를 진행
5. 커널 모드에서 유저 모드로 변경
```

- **프로세스와 스레드는 컨텍스트 스위칭이 발생했을 때 어떤 차이가 있을까요?**
  - 프로세스의 컨텍스트 스위칭 과정에서는 프로세스의 모든 CPU 레지스터 값을 저장해야 하지만, 스레드는 자신의 레지스터 값만 저장하면 됩니다. 
  - 프로세스 간 컨텍스트 스위칭에서는 가상 메모리 주소 매핑이 완전히 변경됩니다. 하지만 스레드 간에는 프로세스 내 메모리 영역을 공유하므로 주소 매핑 변경이 필요 없습니다.
...
- **컨텍스트 스위칭은 언제 일어날까요?**
  - 프로세스 스케쥴링 시 새로운 프로세스가 선택되는 경우와, 타임 슬라이스를 모두 사용했을 경우에도 컨텍스트 스위칭이 일어납니다.
  - 입출력 인터럽트, 트랩 등의 인터럽트가 발생하면 인터럽트 핸들러로 컨텍스트 스위칭이 일어납니다. 인터럽트 처리 후 원래 프로세스로 다시 컨텍스트 스위칭합니다.
  - 프로세스가 입출력, 정지, 대기 등의 이유로 Blocked 상태가 되면 컨텍스트 스위칭이 발생합니다.
  - 사용자 프로세스가 시스템 콜을 통해 커널 모드로 전환되거나 그 반대의 경우에도 컨텍스트 스위칭이 수행됩니다.
  - 멀티스레딩에서 CPU를 할당받을 스레드가 바뀔 때마다 스레드 컨텍스트 스위칭이 일어납니다.
...
<details>
<summary>그렇다면 멀티쓰레딩의 문제점이라고 했을 때 실제 예시는 무엇인가요?</summary>
...
<details>
<summary>멀티쓰레딩의 문제점을 멀티프로세싱이 해결해줄 수 있나요?</summary>

</details>


<details>
<summary>python 을 사용한다고 했을 때, 멀티쓰레딩의 문제를 asyncio 가 해결할 수 있나요?</summary>

</details>


<details>
<summary>asyncio 를 멀티쓰레딩 대신 사용하라고 하는 이유는 무엇일까요?</summary>
